...
TODO : 
    [ ] Clean up and make easy-access dict for data handling 
        (e.g. mezz_resolution, mezz_extension, etc.)
        --see mess at bottom in Encode()
"""
...
class Video():
    ...
    """
    def __init__(self, Settings, mezz_title=None, mezz_filepath=None, video_id=None, **kwargs):
    ...
    def api_determiner(self):
        if self.Settings.NODE_VEDA_ATTACH is False:
            return self
        if self.video_id != None and len(self.Settings.VEDA_API_URL) == 0:
            print 'ERROR : VEDA API Config Incorrect, run test to debug'
            return self
        ...
        data = {
            'edx_id' : self.video_id,
            }
        headers = {'Authorization': 'Bearer ' + veda_token, 'content-type': 'application/json'}
        x = requests.get(self.Settings.VEDA_API_URL + 'videos', params=data, headers=headers) 

        vid_dict = json.loads(x.text)   
        if len(vid_dict['results']) == 0: return self
        ...
        for v in vid_dict['results']:
            ...
            self.mezz_duration = self._seconds_from_string(uni_duration)
            self.mezz_filepath = '/'.join((
                'https://s3.amazonaws.com', 
                self.Settings.MEZZ_INGEST_LOCATION, 
                self.video_id + '.' + self.mezz_extension
                ))
